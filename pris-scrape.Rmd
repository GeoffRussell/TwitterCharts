...
dflo<-inner_join(dfo,dfl,by=join_by(reactor==Name)) |> 
  ...
  summarise(uptime=sum(`Annual Time Online (Hours)`)/(n()*8760))

nreactorsbycountry<-dflo |> select(Country,reactor) |> distinct() |> group_by(Country) |> summarise(n=n())
```

```{r}
cfbycountry<-dflo |> group_by(Country) |>  summarise(avup=mean(uptime,na.rm=T)) |> inner_join(nreactorsbycountry)
r99<-dfo |> filter(name=="Annual Time Online (Hours)" & value>=(0.99*8760)) |> summarise(name=unique(reactor)) |> arrange(name)
r99
```

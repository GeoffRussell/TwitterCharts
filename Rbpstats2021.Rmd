...
makeRollingGraph<-function(rp,ntop=30,rmTypes=c("SolarPlusWindPlusBio","Clean")) {
    ...
    write_csv(mnstmax %>% unnest(c(data,MaxYear,MaxPop)),file="growthtables2021.csv")
    tmpmnst<<-mnstmax 
    ...
    colorVals=c("darkolivegreen3","black","grey","#3399FF","green","darkslategrey","#FFFF00","#FFFF99")
    if (length(rmTypes)>2) {
    ...
    p<-top15 %>% mutate(Key=paste0(Rank,". ",Country,", ",MaxYear)) %>%
      ...
      png(paste0("rollout-top",ntop,"-over-",rp,"-years-",t,".png"),width=1800,height=1300,units="px",res=200,type="cairo-png")
      print(p)
      dev.off()
      print(p)
}

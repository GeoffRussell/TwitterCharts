...
makeRollingGraph<-function(rp) {
    ...
    top15<-mnstmax %>% filter(Type!="SolarPlusWindPlusBio") %>% 
      arrange(desc(Max)) %>% 
      select(Country,Type,MaxYear,Max) %>% ungroup() %>% 
        slice_head(n=30) %>% 
        mutate(Rank=rank(desc(Max))) 
    p<-top15 %>% mutate(Key=paste0(Rank,". ",Country,", ",MaxYear)) %>%
      ggplot(aes(x=reorder(Key,Max),y=Max,fill=Type))+geom_col()+coord_flip()+
      scale_fill_manual(values=c("darkolivegreen3","black","grey","#3399FF","green","darkslategrey","#FFFF00","#FFFF99"))+
      labs(x="",title=paste0("Peak megawatt-hours per person added in ",rp," years"),
           y="Megawatt-hours")

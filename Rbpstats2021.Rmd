...
worldTotalFilterSubChina<-function(.data,name,startyear="1975",endyear="2020...57") {
  rename(.data,Country=`Terawatt-hours`) %>% mutate(Type=name) %>% filter(!is.na(Country)) %>% 
...
makeFilter<-function(cl) {
       function(.data,name,startyear="1975",CL=cl,endyear="2020...57") {
        rename(.data,Country=`Terawatt-hours`) %>% mutate(Type=name) %>% filter(!is.na(Country)) %>% 
...
stwselected<-bind_rows(
    getSheet("Solar Generation - TWh",60,'Solar',startyear="1998",filter=selectedFilter),
    getSheet("Wind Generation - TWh",60,'Wind',startyear="1998",filter=selectedFilter),
    getSheet("Hydro Generation - TWh",60,'Hydro',startyear="1998",filter=selectedFilter)
) %>% calcAndClean()
stwsubch<-bind_rows(
    getSheet("Solar Generation - TWh",60,'Solar',startyear="1998",filter=worldTotalFilterSubChina) %>% 
      mutate(dd=TWh-lag(TWh,23)) %>% filter(Country=="Total World") %>%
      select(Country,Type,Year,dd) %>% mutate(Country="WorldSubChina",TWh=dd) %>% select(-dd),
...
calcGrowth<-function(.data) {
  .data %>% ggplot() + geom_line(aes(x=Year,y=TWh,color=Type,group=Type),size=2)+
...
calcGrowthRatePanel<-function(.data) {
  .data %>% ggplot() + 
  geom_line(aes(x=Year,y=percentGrowth,color=Type,group=Type),size=0.5)+
    facet_wrap(Country~.)+
  labs(title="Percentage growth rates of hydro, solar and wind (Data:BP 2021)",y="Percentage growth")+
...
calcGrowthRate<-function(.data) {
  .data %>% ggplot() + 
  geom_line(aes(x=Year,y=percentGrowth,color=Type,group=Type),size=2)+
  labs(title="Percentage growth rates of hydro, solar and wind",y="Percentage growth")+
  annotate('text',x=2005,y=10,label="Data: BP world energy stats 2021",size=3)+
  theme(axis.title=element_text(size=20,face="bold"),
        axis.text.x=element_text(size=20,face="bold"),
        plot.title=element_text(size=20,face="bold"),
        axis.text.y=element_text(size=20,face="bold"),
...
sgr
png(paste0("selectedCompoundGrowthRates-",selectedLabel,".png"),width=1200,height=1200,units="px",res=300,type="cairo-png")
sgr
dev.off()

wshGrowthRate<-calcGrowthRate(stwh)

wshGrowthRate

# and without china
wgrsubc<-calcGrowthRate(stwsubch)
wgrsubc

png(paste0("wsh-growth-2000-2020.png"),width=1800,height=1300,units="px",res=200,type="cairo-png")
print(wshGrowth)
dev.off()
png(paste0("wsh-growthrate-2000-2020.png"),width=2600,height=1300,units="px",res=200,type="cairo-png")
print(wshGrowthRate)
dev.off()
...
#---------------------------------------------------------------------------
fulldfEx<-twhEx %>% mergePopData(clist$Country) %>% 
  mutate(MWhPerCap=TWh*1e12/Popn/1e6) # NB this is over the period
```
...
makeRollingGraph<-function(rp,ntop=30,rmTypes=c("SolarPlusWindPlusBio","Clean")) {
    ...
    cleanData <- fulldfEx %>% filter(Type=="Clean",Year=="2020") %>% 
      select(Country,TWh,Popn) %>% mutate(cleanMWhPerCap=TWh*1e12/Popn/1e6) %>% select(-TWh,-Popn)
    tmpClean<<-cleanData
    mnstmax<-mnstmax %>% left_join(cleanData)
    write_csv(mnstmax %>% unnest(c(data,MaxYear,MaxPop)),file=paste0("growthtables2021-",rp,".csv"))
    saveRDS(mnstmax,file=paste0("growthtables2021-",rp,".rds"))
    tmpmnst<<-mnstmax 
    ...
    top15<-mnstmax %>% filter(!Type %in% rmTypes) %>% 
      arrange(desc(Max)) %>% 
    ...
    p<-top15 %>% mutate(Key=paste0(Rank,". ",Country,", ",MaxYear,", ",comma2(as.numeric(MaxPop)/1e6),"m")) %>%
      ...
      annotate('text',x=2,y=20,hjust=0,label="Data: BP World Energy Stats 2021",size=2)+
      ...
      png(paste0("rollout-top",ntop,"-over-",rp,"-years-",t,".png"),width=2200,height=1300,units="px",res=300,type="cairo-png")
      print(p)
      dev.off()
      print(p)
}

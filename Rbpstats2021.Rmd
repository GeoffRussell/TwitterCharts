...
makeRollingGraph<-function(rp,ntop=30) {
    ...
    top15<-mnstmax %>% filter(!Type %in% c("SolarPlusWindPlusBio","Clean")) %>% 
      arrange(desc(Max)) %>% 
    ...
    p<-top15 %>% mutate(Key=paste0(Rank,". ",Country,", ",MaxYear)) %>%
      ...
      geom_point(aes(y=totalMWhPerCap*1.5,shape="2050 Target"),alpha=0.8,size=2,fill="red") +
      geom_point(aes(y=totalMWhPerCap,shape="Current MWh/Cap"),alpha=0.8,size=1) +
      geom_point(aes(y=cleanMWhPerCap,shape="Current Clean"),alpha=0.8,size=2) +
      geom_col(alpha=1)+
      geom_text(aes(y=30,label=paste0(comma2(Max),", ",comma3(as.numeric(MaxPop)/1e6),"m"),hjust=0),size=3)+ coord_flip()+
      ...
      labs(x="",title=paste0("Highest megawatt-hours per person added in ",rp," years"),
           y="Megawatt-hours increment (barend text MWh/cap,population in last year)")+ylim(c(0,34))+
      annotate('text',x=2,y=10,hjust=0,label="Data: BP World Energy Stats 2021",size=3)+
      scale_shape_manual(values=c(19,8,1),labels=c("2050 Target",
                                                   "Current Clean", "Current MWh/Cap"
                                                   ))+

...
makeRollingGraph<-function(rp,ntop=30,rmTypes=c("SolarPlusWindPlusBio","Clean")) {
    ...
    p<-top15 %>% mutate(Key=paste0(Rank,". ",Country,", ",MaxYear,", ",comma2(as.numeric(MaxPop)/1e6),"m")) %>%
      ...
      geom_rect(aes(ymin=cleanMWhPerCap,ymax=totalMWhPerCap*2,
      ...
      geom_text(aes(y=totalMWhPerCap*2+1,label=paste0(comma2((totalMWhPerCap*2-cleanMWhPerCap)/Max),"x",rp," yrs"),hjust=0),size=3)+ 
      coord_flip()+
      scale_fill_manual(values=colorVals)+
      labs(x="",title=paste0("Highest megawatt-hours per person added in ",rp," years"),
           y="Megawatt-hours increment (barend text MWh/cap,population in last year)")+ylim(c(0,40))+

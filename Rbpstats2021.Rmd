...
#-------------------------------------------------------------------------
write_csv(fulldfEx,file="growthtables2021.csv")
...
makeRollingGraph<-function(rp,ntop=30) {
 ...
 #               mergeTotalTWhData(nst$Country %>% unique) %>%
 #               mutate(totalMWhPerCap=totalTWh*1e12/Popn/1e6) # NB this is an annual figure
    ...
    top15<-mnstmax %>% filter(Type!="SolarPlusWindPlusBio") %>% 
      arrange(desc(Max)) %>% 
      select(Country,Type,MaxYear,Max,MaxPop,totalMWhPerCap) %>% ungroup() %>% 
        slice_head(n=ntop) %>% 
        mutate(Rank=rank(desc(Max))) 
    p<-top15 %>% mutate(Key=paste0(Rank,". ",Country,", ",MaxYear)) %>%
      mutate(therow=reorder(Key,Max)) %>%
      ggplot(aes(x=therow,y=Max,fill=Type))+
      geom_point(aes(y=totalMWhPerCap,shape="Current MWh/Cap"),alpha=0.3,size=2) +
      geom_point(aes(y=totalMWhPerCap*1.5,shape="2050 Target"),alpha=0.3,size=3,color="red") +
      #geom_rect(aes(xmin=therow-0.5,xmax=therow+0.4,ymin=totalMWhPerCap-1,ymax=totalMWhPerCap+1),fill="red",alpha=0.3) +
      geom_col()+
      ...
      labs(x="",title=paste0("Highest megawatt-hours per person added in ",rp," years"),
           y="Megawatt-hours (barend text MWh/cap,population)")+ylim(c(0,30))+
      annotate('text',x=3,y=10,hjust=0,label="Data: BP World Energy Stats 2021",size=3)+
      scale_shape_manual(values=c(19,1),labels=c("2050 Target","Current MWh/Cap"))+

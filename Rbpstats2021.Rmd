...
makeRollingGraph<-function(rp) {
    ...
    p<-top15 %>% mutate(Key=paste0(Rank,". ",Country,", ",MaxYear)) %>%
      ...
      geom_col()+
      geom_text(aes(y=Max+0.2,label=paste0(comma3(as.numeric(MaxPop)/1e6),"m"),hjust=0))+
      coord_flip()+
      ...
      labs(x="",title=paste0("Highest megawatt-hours per person added in ",rp," years"),
           y="Megawatt-hours (barend text is population)")+ylim(c(0,25))
      png(paste0("rollout-top-over-",rp,"-years.png"),width=1800,height=1300,units="px",res=200,type="cairo-png")
      print(p)
      dev.off()
      print(p)
}

...
xxxx<-getSheet("Solar Generation - TWh",60,'Solar',startyear="1998",filter=worldTotalFilterSubChina) %>% mutate(dd=TWh-lag(TWh,23)) %>% filter(Country=="Total World") %>% select(Country,Type,Year,dd) %>% mutate(Country="WorldSubChina",TWh=dd) %>% select(-dd) 
```
...
stwsubch<-bind_rows(
    ...
    %>% mutate(dd=TWh-lag(TWh,23)) %>% filter(Country=="Total World") %>% select(Country,Type,Year,dd) %>% mutate(Country="WorldSubChina",TWh=dd) %>% select(-dd),
...
pg<-calcGrowth(stwh)
pg
pgc<-calcGrowth(stwsubch)
pgc

wshGrowth<-stwh %>% ggplot() + geom_line(aes(x=Year,y=TWh,color=Type,group=Type),size=2)+
  labs(title="Growth of hydro, solar and wind",y="Terawatt-hours")+
  annotate('text',x=2000,y=300,hjust=0,label="Data: BP world energy stats 2021",size=3)+
  theme(axis.title=element_text(size=20,face="bold"),
        axis.text.x=element_text(size=20,face="bold"),
